...
class InsFinancialReport(models.TransientModel):
        ...
        for report in child_reports:
            ...
            if res[report.id].get('account'):
                ...
                for account_id, value in res[report.id]['account'].items():
                    ...
                    vals = {
                        'account': account.id,
                        'name': f"{account.code} {account.name}" if account.code else (account.name or ''),
    ...
    def get_report_values(self):
        ...
        # Write computed dates back (onchange only sets in-memory)
        write_vals = {}
        if self.date_from:
            write_vals['date_from'] = self.date_from
        if self.date_to:
            write_vals['date_to'] = self.date_to
        if write_vals:
            self.write(write_vals)
        import logging
        _logger = logging.getLogger(__name__)
        _logger.info("=== get_report_values: date_from=%s date_to=%s company=%s report=%s journals=%s target=%s",
                     self.date_from, self.date_to, self.company_id.name,
                     self.account_report_id.name, self.journal_ids.ids, self.target_move)
        report_lines = self.get_account_lines()
        _logger.info("=== get_report_values: got %d lines", len(report_lines))
        return report_lines
    ...
    def _compute_display_name(self):
        for record in self:
            record.display_name = record.account_report_id.name or _('Financial Report')

    @api.depends('account_report_id')
    def name_get(self):
        res = []
        for record in self:
            name = record.account_report_id.name or _('Financial Report')
    ...
    def prepare_values_for_component(self):
        ...
        # Write computed dates back to the record (onchange only sets in-memory values)
        write_vals = {}
        if self.date_from:
            write_vals['date_from'] = self.date_from
        if self.date_to:
        ...
        if self.comparison_date_from:
            write_vals['comparison_date_from'] = self.comparison_date_from
        if self.comparison_date_to:
            write_vals['comparison_date_to'] = self.comparison_date_to
        if write_vals:
            self.write(write_vals)
